...
def collate_fn(batch):
    if len(batch) == 0:
        print("Returning emoty sequences, Hopefully you never see this!")
    ...
    inputs_embeds = torch.zeros(len(batch), max_len, hidden_dim, dtype=dtype)
    lengths = []
    ...
    # Compute attention_mask: 1 for real tokens, 0 for padding (right-padded)
    attention_mask = (torch.arange(max_len).unsqueeze(0) < lengths.unsqueeze(1)).to(dtype=torch.long)  # Shape: (batch_size, max_len)

    # Print after padding -- this will show for each batch
    #print(f"[COLLATE] Padded to: {max_len}")
    #print(f"          inputs_embeds shape: {inputs_embeds.shape}")
    #print(f"          Example attention_mask (for this batch):\n{attention_mask.tolist()}\n")  # Prints as nested list, e.g., [[1,1,1,0,0], ...]
    return inputs_embeds, lengths, attention_mask
...
def train_epoch(model, loader, optimizer, criterion, device, epoch):
    ...
    for i, batch in enumerate(loader):
        seqs, lengths, attn_mask = batch
        seqs = seqs.to(device)
        lengths = lengths.to(device)
        attn_mask = attn_mask.to(device).float()

        #if i == 0:
        #    print(f"[TRAIN] Example inputs_embeds (first seq, first 3 tokens):\n{seqs[0, :3].tolist()}")
        ...
        loss, mse, cos = criterion(recon[0], seqs, lengths)
        loss.backward()
...
def main():
    ...
    if cfg_m.BL_type == "linear":
        ...
        print("Instaiating Bottleneck linear...")
        model = AdaptiveBottleneck(ratio=cfg_m.bl_ratio)
    elif cfg_m.BL_type == "attention":
        print("Instaiating Bottleneck Attention...")            
        ...
        model = BottleneckAttention(config_BL, ratio=cfg_m.bl_ratio)
