...
from SVD_GPTtest import GPT2LMHeadModelSVD
...
def plot_metrics(metrics_history, save_path=f"./training_metrics_SVD_r{cfg_m.r_LoRA}_.png"):
    """
...
def main():
    ...
    #parser.add_argument("--path_BL", type=str, default="/home/oscar.berg/compressGPT/checkpoints_bottleneck/_r_16_pos_3_type_linear/best.pt",  help="Path to traind BL architecture")
    ...
    print("\n" + "="*80)
    print("INITIALIZING MODEL & TOKINIZER")
    print("="*80)
    ...
    model = GPT2LMHeadModelSVD(config, svd_r=cfg_m.svd_r)
    # Resize embeddings if needed
    if len(tokenizer) != model.config.vocab_size:
        model.resize_token_embeddings(len(tokenizer))

    # Copy weights from official model's transformer
    model.transformer.load_state_dict(official_model.transformer.state_dict(), strict=False)  # strict=False to enable Bottleneck layer loading
    ...
    if cfg_m.svd_en:
        for i in range(12):
            model.transformer.h[i].attn.translate_weights()
            delattr(model.transformer.h[i].attn, 'c_attn')
            model.transformer.h[i].attn.translate_SVD()
            delattr(model.transformer.h[i].attn, 'WK')
            delattr(model.transformer.h[i].attn, 'WV')
    ...
    lora_config = LoraConfig(
        r=cfg_m.r_LoRA,  # Low-rank dimension for LoRA adapters (e.g., 8 or 16)
        lora_alpha=16,
        target_modules=["c_fc", "c_proj", "WQ", "WK_A", "WK_B", "WV_A", "WV_B"],     # Targets Q projections (WQ_H), FFN (c_fc, c_proj), and attn output (c_proj)
        
        rank_pattern={
            "c_fc": cfg_m.r_LoRA,      # FFN layer gets rank
            "c_proj": cfg_m.r_LoRA,    # Projection layer gets rank
            "WK_A": 3,
            "WK_B": 3,
            "WV_A": 3,
            "WV_B": 3,
    ...
    # Apply LoRA to the specified modules (this will freeze the base model and add trainable LoRA params)
    model = get_peft_model(model, lora_config)

    # Now, explicitly make the SVD-decomposed K and V matrices trainable (WK_A/B, WV_A/B)
    # This overrides the freeze from peft
    #for name, param in model.named_parameters():
    #    if any(sub in name for sub in ["WK_A", "WK_B", "WV_A", "WV_B"]):
    ...
    if cfg_m.default:
        ...
        save_path = f"./metrics_svd_r_{cfg_m.svd_r}.png"
        final_dir = f"./gpt2-alpaca-final_svd_r_{cfg_m.svd_r}.png"
    ...
    from pretrain_attnBottleneck_withinModel import save_checkpoint
    # Add the checkpoint save (adjust optimizer, epoch, val_loss, args as per your training loop)
    checkpoint_path = os.path.join(final_dir, "checkpoint.pth")  # Or any filename you prefer
    save_checkpoint(model, optimizer, epoch, val_loss, checkpoint_path, args)
